...
export class OrdersService {
  ...
  async UpdateNbItemReturnedForOrderProduct(
    nbItemReturned: number,
    idOrderProduct: string,
  ) {
    return await this.orderProductRepository.update(
      { id: idOrderProduct['id'] },
      { nbProductReturned: nbItemReturned['quantity'], is_returned: true },
    );
  }
  ...
  async CreateOrder(data: CreateOrderDto): Promise<Order> {
    try {
      ...
      const createdOrderProducts = orderProducts.map(async (item) => {
        return await this.orderProductRepository.save(item);
      });

      let orderProductsIdsConcatenation: string;

      await Promise.all(
        createdOrderProducts.map((orderProduct) =>
          orderProduct.then((item) => item.id),
        ),
      ).then((ids) => {
      ...
      this.ordersRepository.update(
        { id: order.id },
        { orderProducts: orderProductsIdsConcatenation },
      );
      return createdOrder;
    } catch (error) {
      throw new HttpException(
  ...
  async GetOrderProductsByProductsIds(
    productIds: Array<string>,
  ): Promise<Array<OrderResponseDto>> {
    let orderProducts: Array<OrderProduct> = [];
    try {
      orderProducts = await this.orderProductRepository.find({
        where: {
    ...
    const result: Array<OrderResponseDto> = await Promise.all(
      orderProducts.map(async (elm) => {
        const orderProduct: OrderProduct = elm;
        const order: Order = await this.ordersRepository.findOneBy({
        ...
        orderProducts.push(
          new OrderProductDto(
            orderProduct?.id,
            new Product(orderProduct?.product_id),
            orderProduct?.quantity,
            orderProduct?.is_returned,
            undefined,
            orderProduct?.nbProductReturned,
          ),
        );
  ...
  async GetOrdersWithProducts(
    ...
    try {
      ...
      for (const order of orders) {
        ...
        const products: OrderProductDto[] = await Promise.all(
          orderProductIds.map(async (item) => {
            const orderProduct: OrderProduct =
              await this.orderProductRepository.findOneBy({ id: item });
