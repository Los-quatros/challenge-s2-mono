...
export class ReturnsService {
  ...
  async Cretae(returnModel: CreateReturnModel, userId: string) {
    try {
      const orderProductsIdsConcatened: string = returnModel['orderProducts']
        .map((elm) => {
  ...
  async ValidateOrDecline(decision: boolean, returnId: string) {
    if (decision) {
      return this.returnsRepository.update(
        { id: returnId },
        { status: 'validated' },
      );
    } else {
  ...
  async GetAllForSeller(orderProductsSeller: Array<string>) {
    ...
    const returnsDto: Array<ReturnsResponseDto> =
      await this.BuildReturnsDtoFromReturns(allReturns);
    const returnsContainingSellerProducts: Array<ReturnsResponseDto> =
      returnsDto
        .map((elm: ReturnsResponseDto) => {
          if (
            elm['orderProducts'].some((product: string) =>
              orderProductsSeller.includes(product),
            )
          ) {
            return elm;
          }
    ...
    const result: Array<ReturnsResponseDto> =
      returnsContainingSellerProducts.map((elm: ReturnsResponseDto) => {
        elm.orderProducts = elm.orderProducts.filter((op) =>
          orderProductsSeller.includes(op),
        );
        return elm;
      });
    return result;
  ...
  // TODO : A TESTER
  private async BuildReturnsDtoFromReturns(
    returns: Array<Returns>,
  ): Promise<Array<ReturnsResponseDto>> {
    return Promise.all(
      returns.map(async (elm: Returns) => {
