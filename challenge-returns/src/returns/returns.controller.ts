...
export class ReturnsController {
    ...
    async CreateReturn(@Payload() createReturnModel : CreateReturnModel, @Payload() userId : string) {
        return this.returnsService.Cretae(createReturnModel['retrunDto'], userId['userId']);
    }

    @EventPattern('AcceptOrDeclineReturn')
    async ModerateReturn(@Payload() data : any){
        return this.returnsService.ValidateOrDecline(data['decision'], data['returnId']);
    }
